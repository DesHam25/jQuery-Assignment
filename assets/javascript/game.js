...
for(var i =0; i < 4; i++){ 
    var random = Math.floor(Math.random() * 11) + 1; //this is random number for every single Vibranium
    // console.log(random);
    //this is creating a div for the random number
    var Vibranium = $("<div>");
        Vibranium.attr({ //attribute into one class on random
            "class": 'Vibranium',
            "data-random": random
        });
            Vibranium.css({
...
$(document).on('click', ".Vibranium", function () {
    ...
    //we Have to do a conditional logic if its greater than random Number which is (var random_Number) we need to make a if statement
});
//Pseudo coding 
